    ...
    <div className="max-w-6xl mx-auto space-y-6 animate-fade-in pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 pt-4 px-2 md:px-0">
        <div>
          ...
          <p className="text-slate-500 font-medium text-sm mt-1">Here is what's happening today.</p>
        </div>
        
        {/* Minimalist Daily Mission */}
        <div className="flex items-center justify-between md:justify-end gap-6 bg-white py-3 px-6 rounded-2xl shadow-[0_4px_20px_-5px_rgba(0,0,0,0.05)] border border-slate-100">
           ...
           <div>
              <div className="text-[10px] font-bold text-slate-400 tracking-widest uppercase">Hot Leads</div>
      ...
      <div className="bg-white rounded-3xl shadow-[0_8px_30px_-5px_rgba(0,0,0,0.03)] border border-slate-100 overflow-hidden">
        <div className="grid grid-cols-2 md:grid-cols-4 divide-y md:divide-y-0 md:divide-x divide-slate-100">
          {[
            { label: 'Total Contacts', value: clients.length, color: 'text-indigo-600', filter: 'All' },
            ...
            { label: 'Passed', value: notInterested, color: 'text-slate-400', filter: 'Not interested' },
          ].map((stat, i) => (
            <button 
              key={i} 
              onClick={() => onFilterView && onFilterView(stat.filter)}
              className="p-6 md:p-8 flex flex-col items-center md:items-start text-center md:text-left hover:bg-slate-50/50 transition-colors group"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pt-4">
        ...
        <div className="lg:col-span-2 bg-white rounded-3xl p-6 md:p-8 shadow-[0_8px_30px_-5px_rgba(0,0,0,0.03)] border border-slate-100 flex flex-col h-[400px]">
          <div className="flex items-center justify-between mb-8">
            <h3 className="text-base font-bold text-slate-800 flex items-center gap-2">
               <Activity className="w-4 h-4 text-emerald-500" /> Activity Trend
            </h3>
            <div className="flex items-center gap-4 text-[10px] font-bold uppercase tracking-widest text-slate-400">
               <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-slate-200"></div> Leads</div>
               <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-emerald-400"></div> Interactions</div>
            </div>
          </div>
            ...
            {activityData.map((data, i) => (
              <div key={i} className="flex flex-col items-center flex-1 h-full justify-end gap-3 group">
                <div className="flex justify-center gap-1.5 w-full items-end flex-1">
                   <div className="relative w-1/3 max-w-[12px] bg-slate-50 rounded-t-sm overflow-hidden flex items-end h-full">
                      <div className="w-full bg-slate-200 transition-all duration-1000 group-hover:bg-slate-300" style={{ height: `${(data.leads / maxActivity) * 100}%`, minHeight: data.leads > 0 ? '4px' : '0' }} />
                   </div>
                   <div className="relative w-1/3 max-w-[12px] bg-slate-50 rounded-t-sm overflow-hidden flex items-end h-full">
                      <div className="w-full bg-emerald-400 transition-all duration-1000 group-hover:bg-emerald-500" style={{ height: `${(data.interactions / maxActivity) * 100}%`, minHeight: data.interactions > 0 ? '4px' : '0' }} />
                   </div>
                </div>
        ...
        <div className="flex flex-col gap-6">
           ...
           {/* Meetings */}
           <div className="bg-white rounded-3xl p-6 md:p-8 shadow-[0_8px_30px_-5px_rgba(0,0,0,0.03)] border border-slate-100 flex-1">
             ...
             <div className="space-y-4">
               {todayMeetings.length > 0 ? todayMeetings.slice(0, 4).map(c => (
                 <div key={c.id} className="group flex items-center justify-between cursor-pointer" onClick={() => onSelectClient(c.id)}>
                   <div>
                     <div className="font-bold text-sm text-slate-700 group-hover:text-indigo-600 transition-colors">{c.name}</div>
                     <div className="text-xs text-slate-400 mt-0.5">{new Date(c.meetingDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                   </div>
                   <ChevronRight className="w-4 h-4 text-slate-300 group-hover:text-indigo-400 transition-colors" />
                 </div>
               )) : <div className="text-sm font-medium text-slate-400 py-4">No meetings today.</div>}
             </div>
           </div>
           ...
           <div className="bg-white rounded-3xl p-6 md:p-8 shadow-[0_8px_30px_-5px_rgba(0,0,0,0.03)] border border-slate-100 flex-1">
             ...
             <div className="space-y-4">
               {upcomingFollowUps.length > 0 ? upcomingFollowUps.slice(0, 4).map(c => (
                 <div key={c.id} className="group flex items-center justify-between cursor-pointer" onClick={() => onSelectClient(c.id)}>
                   <div>
                     <div className="font-bold text-sm text-slate-700 group-hover:text-amber-600 transition-colors">{c.name}</div>
               ...
               )) : <div className="text-sm font-medium text-slate-400 py-4">Inbox Zero.</div>}
             </div>
           </div>
